...
const DEFAULT_OPTS = { count: 0, force: false, dryRun: false }

function migrate (set, direction, migrationName, fn, context, opts) {
  let { count, force, dryRun } = { ...DEFAULT_OPTS, ...(opts || {}) }
  ...
  function next (migration) {
    ...
    if (dryRun) {
      ...
      var returnValue = migration[direction](context, function (err) {
